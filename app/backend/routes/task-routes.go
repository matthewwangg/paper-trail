...
func SetupTaskRoutes(router *gin.Engine) {
	...
	tasks.Use(middleware.AuthMiddleware())
	{
		...
		tasks.GET("/grouped/status", tollbooth_gin.LimitHandler(limiter), controllers.GetTasksGroupedByStatus)
		tasks.GET("/grouped/priority", tollbooth_gin.LimitHandler(limiter), controllers.GetTasksGroupedByPriority)
